...
with sqlite3.connect('DisneyDB2.db') as conn:
    cursor = conn.cursor()
    Info = cursor.execute('Select RaceDetails.RaceType, RaceDetails.Year from Race Inner Join RaceDetails on Race.RaceID=RaceDetails.RaceID where RunnerID=?;', [Runner])
